...
def getSelectedEvents(filename):
    ...
    _files.add(f)
    hist_selected_MuMu = f.Get("jj_M_MuMu_hZA_lljj_deepCSV_btagM_no_cut")
    hist_selected_ElEl = f.Get("jj_M_ElEl_hZA_lljj_deepCSV_btagM_no_cut")
...
def main():
    ...
    with open('acceptance.csv', 'wb') as csvfile:
        ...
        for i, filename in enumerate(glob.glob(os.path.join(args.path, '*.root'))):
            ...
            spamwriter.writerow([float(MA), float(MH), acceptance])
