...
def drawTheoryGraph(gr, tb):
    ...
    pente = math.atan( 420/3.3 *(log10(y2)-log10(y1))/(x2-x1))
...
gr_tb1_5 = file_theory.Get("Xsec_mH"+str(mH)+"_tb1_5")
...
pente = math.atan( 420/3.3 *(log10(y2)-log10(y1))/(x2-x1))
#pente = 420/3.3 * math.atan((log10(y2)-log10(y1))/(x2-x1))

print y1, y2, x1, x2, (log10(y2)-log10(y1))/(x2-x1),  pente

size = 5
x_pl = numpy.array([x1+size, x2+size, x2-size, x1-size, x1+size], dtype=float) #[x1+10, x2+10, x2-10, x1-10, x1+10]
y_pl = numpy.array([y1+size, y2+size, y2-size, y1-size, y1+size], dtype=float)
pline = TPolyLine(5,x_pl,y_pl)
pline.SetFillColor(kWhite)
pline.Draw("f")

l = TLatex(x1,y1,"tan #beta = 1")
l.SetTextSize(20)
l.SetTextFont(43)
l.SetTextAlign(12)
l.SetTextAngle(pente*180/math.pi)
l.SetTextColor(kGray+1)
l.Draw()
'''
#gPad().SetLogy(1)
...
if run_blind :
    C.Print("BBP"+str(mH)+"_blind_v2.pdf")
    C.Print("BBP"+str(mH)+"_blind_v2.png")
    f = TFile("narrow_BBP"+str(mH)+"_blind_v2.root","recreate")
else :
    C.Print("BBP"+str(mH)+"_v2.pdf")
    C.Print("BBP"+str(mH)+"_v2.png")
